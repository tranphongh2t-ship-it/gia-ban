...
export default function TinhGiaMelamineTonghopPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await apiGet('/gia-chuan/tinh-gia-melamine-tonghop')
      setData(res.data || [])
    } catch { setData([]) }
  ...
  const handleAutoAssign = async () => {
    setAssigning(true)
    try {
      const res = await apiPost('/gia-chuan/tinh-gia-melamine-tonghop/auto-assign-ma-sp', {})
      alert(`Đã gán ${res.assigned} mã SP từ danh mục MISA (${res.skipped} dòng không có mã để trống)`)
      fetchData()
    } catch (e: any) { alert('Lỗi: ' + e.message) }
  ...
  return (
    <div style={pageContainer}>
      <h1 style={pageTitle}>Tính Giá VÁN NHỰA - PLYWOOD - OSB - GỖ GHÉP</h1>
      ...
      <div style={{ background: colors.card, borderRadius: radius.lg, padding: 20, border: `1px solid ${colors.border}`, boxShadow: shadow.card, marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ minWidth: 200, flex: 1 }}>
            ...
            <input style={inputStyle} placeholder="Mã màu, loại cốt, độ dày..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          ...
          <div style={{ minWidth: 180 }}>
            ...
            <select style={inputStyle} value={filterBang} onChange={e => setFilterBang(e.target.value)}>
              <option value="">Tất cả</option>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 800 }}>
              <thead>
                <tr style={{ background: colors.surfaceSecondary }}>
                  ...
                  <th style={{ padding: '10px 14px', textAlign: 'left', color: colors.textMuted, fontWeight: 600, fontSize: 11, textTransform: 'uppercase', borderBottom: `1px solid ${colors.border}`, whiteSpace: 'nowrap' }}>Phân nhóm</th>
                  <th style={{ padding: '10px 14px', textAlign: 'left', color: colors.textMuted, fontWeight: 600, fontSize: 11, textTransform: 'uppercase', borderBottom: `1px solid ${colors.border}`, whiteSpace: 'nowrap' }}>Bảng</th>
                  <th style={{ padding: '10px 14px', textAlign: 'left', color: colors.textMuted, fontWeight: 600, fontSize: 11, textTransform: 'uppercase', borderBottom: `1px solid ${colors.border}`, whiteSpace: 'nowrap' }}>Loại cốt</th>
                  ...
                  <tr key={r.id || i} style={{ background: i % 2 === 0 ? colors.card : colors.surfaceSecondary }}>
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: r.ma_sp ? colors.primary : colors.textMuted, fontWeight: 600, fontFamily: 'monospace', fontSize: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <AssignMisaCode module="melamine_tonghop" table="bang_gia_chuan_tinh_gia_melamine_tonghop" rowId={r.id} searchStr={`${r.bang} ${r.loai_cot} ${r.do_day} ${r.ma_mau} ${r.so_mat} mặt`} currentMa={r.ma_sp} currentTen={r.ten_sp} onAssigned={fetchData} />
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: colors.textSecondary, fontSize: 12 }}>{r.phan_nhom}</td>
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: colors.text, fontSize: 12 }}>{r.bang}</td>
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: colors.text, fontSize: 12 }}>{r.loai_cot}</td>
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: colors.text, fontSize: 12 }}>{r.do_day}</td>
                    <td style={{ padding: '8px 14px', textAlign: 'center', borderBottom: `1px solid ${colors.borderLight}`, color: colors.text, fontSize: 12 }}>{r.so_mat}</td>
