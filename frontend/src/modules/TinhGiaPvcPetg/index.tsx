...
export default function TinhGiaPvcPetgPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await apiGet('/gia-chuan/tinh-gia-pvc-petg')
      setData(res.data || [])
    } catch { setData([]) }
  ...
  useEffect(() => { setPage(0) }, [search, filterNhom, filterLoaiVan, filterSoMat])

  return (
    <div style={pageContainer}>
      <h1 style={pageTitle}>Tính Giá Ván Phủ PVC FILM - PETG</h1>
      ...
      <div style={{ background: colors.card, borderRadius: radius.lg, padding: 20, border: `1px solid ${colors.border}`, boxShadow: shadow.card, marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ minWidth: 180, flex: 1 }}>
            ...
            <input style={inputStyle} placeholder="Mã màu, loại ván, độ dày..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
              ...
              <tbody>
                {pageRows.map((r, i) => (
                  <tr key={r.id || i} style={{ background: i % 2 === 0 ? colors.card : colors.surfaceSecondary }}>
                    ...
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, fontFamily: 'monospace', fontSize: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ whiteSpace: 'nowrap' }}>{r.ma_sp || '—'}</span>
                        <AssignMisaCode module="pvc_petg" table="bang_gia_chuan_tinh_gia_pvc_petg" rowId={r.id} searchStr={`${r.loai_van} ${r.do_day} ${r.ma_mau} ${r.nhom} ${r.so_mat}`} currentMa={r.ma_sp} currentTen={r.ten_sp} onAssigned={fetchData} />
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: colors.textSecondary, fontSize: 12 }}>{r.loai}</td>
      ...
      <GuideTabs title="Hướng dẫn" tabs={pvcPetgGuideTabs} />
    </div>
  )
}
