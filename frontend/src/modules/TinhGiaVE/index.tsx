...
export default function TinhGiaVEPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await apiGet('/gia-chuan/tinh-gia-ve')
      setData(res.data || [])
    } catch { setData([]) }
  ...
  const handleCompute = async () => {
    setComputing(true)
    try {
      const res = await apiPost('/gia-chuan/tinh-gia-ve/tinh-toan', {})
  ...
  const handleAutoAssign = async () => {
    setAssigning(true)
    try {
      const res = await apiPost('/gia-chuan/tinh-gia-ve/auto-assign-ma-sp', {})
  ...
  const filtered = data.filter(r => {
    if (search) {
      ...
      if (!r.quy_cach?.toLowerCase().includes(s) && !r.loai?.toLowerCase().includes(s) && !r.ma_sp?.toLowerCase().includes(s) && !r.ten_sp?.toLowerCase().includes(s)) return false
    }
  ...
  useEffect(() => { setPage(0) }, [search, filterNhom, filterLoai])

  return (
    <div style={pageContainer}>
      <h1 style={pageTitle}>Tính Giá Gốc Ván Ép</h1>
                  ...
                  <tr key={r.id || i} style={{ background: i % 2 === 0 ? colors.card : colors.surfaceSecondary }}>
                    <td style={{ padding: '8px 14px', borderBottom: `1px solid ${colors.borderLight}`, color: r.ma_sp ? colors.primary : colors.textMuted, fontWeight: 600, fontFamily: 'monospace', fontSize: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <AssignMisaCode module="ve" table="bang_gia_chuan_tinh_gia_ve" rowId={r.id} searchStr={`${r.nhom} ${r.loai} ${r.quy_cach}`} currentMa={r.ma_sp} currentTen={r.ten_sp} onAssigned={fetchData} />
                      </div>
                    </td>
      ...
      <GuideTabs title="Hướng dẫn" tabs={veGuideTabs} />
    </div>
  )
}
